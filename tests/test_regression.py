...
# Mode (a) cases whose references only reproduce on the machine that generated
# them (RJC's other machine). Skipped only when --skip-machine-dependent is
# given, so the reference machine and CI stay strict; this is an opt-out for
# working on a second machine, not a relaxation of the harness.
#
# Diagnosed 2026-07-30: NOT a regression. Extracting commit 182bc20 (end of
# Stage 3.5, before any Stage 4 work) and running J0903p2628 against it gives
# chi-squared 5111.503747, bit-identical to current code, versus a reference of
# 5057.568013. What diverges is overwhelmingly the *covariance*: all nine fail
# compare_covar, only one also misses the 1% chi-squared band, and every case
# still passes the 0.1-sigma parameter check. That is the expected signature of
# a finite-difference Jacobian -- the covariance is (J^T J)^-1 with J
# differenced at sqrt(eps), so its ~1e-8 relative noise is amplified by the
# condition number of J^T J, which is large for these blended, near-degenerate
# real-world fits. The mode (b) fixed-parameter evaluations, which are the
# primary gate (Q11), pass for all nine.
MINIMISATION_MACHINE_DEPENDENT = {
    "context/fitting_examples/DH/HS0105p1619/model/HS0105p1619",
    "context/fitting_examples/DH/J1358p0349/model/J1358p0349",
    "context/fitting_examples/DH/J1358p6522/model/J1358p6522_original",
    "context/fitting_examples/DH/J1419p0829/model/J1419p0829",
    "context/fitting_examples/DH/J1558m0031/model/J1558m0031_FINAL_MODEL",
    "context/fitting_examples/DH/Q0913p072/model/Q0913p072",
    "context/fitting_examples/DH/Q1243p307/model/Q1243p307_converge_newstart76",
    "context/fitting_examples/VMP_DLA/J0035m0918/model/J0035m0918_all_data",
    "context/fitting_examples/VMP_DLA/J0903p2628/model/J0903p2628",
}


def _needs_linetools(case):
    """
    Return True if the case's model uses the ``lsf`` function.

    That function wraps ``linetools.spectra.lsf.LSF``, and linetools is an
    optional dependency (nothing else in ALIS needs it), so on a machine
    without it these cases cannot run and are skipped rather than failed --
    see the ``linetools`` marker in conftest.

    Matching the literal ``lsf(`` is enough to distinguish it from the
    ``lsfspline(`` and ``lsffile(`` functions, which are pure ALIS.

    Generated by RJC and Claude.
    """
    try:
        return "lsf(" in case.mod_file.read_text()
    except OSError:
        return False


def _params(cases, batch=None, overrides=None, machine_dependent=None):
    """
    ...
    machine_dependent : set[str] | None
        Case names to tag with the ``machine_dependent`` marker, which
        ``--skip-machine-dependent`` skips. Passed only for the
        minimisation call site; the fixed-parameter gate stays strict.
    ...
    machine_dependent = machine_dependent or set()
    params = []
    for case in cases:
        ...
        marks = [mark, source_mark]
        if _needs_linetools(case):
            marks.append(pytest.mark.linetools)
        if case.name in machine_dependent:
            marks.append(pytest.mark.machine_dependent)
...
@pytest.mark.parametrize(
    "case",
    _params(
        [c for c in _FIT_CASES if c.name not in MINIMISATION_KNOWN_DIVERGENCE],
        machine_dependent=MINIMISATION_MACHINE_DEPENDENT,
    ),
)
