...
#		msgs.info("Using {0:d} CPUs".format(slf._argflag['run']['ncpus']),verbose=slf._argflag['out']['verbose'])
        ...
        mr = alfit(p0new, parinfo=parinfo, functkw=fa,
                   ...
                   ncpus=slf._argflag['run']['ncpus'], ngpus=slf._argflag['run']['ngpus'], backend=slf._argflag['run']['backend'], gputhresh=slf._argflag['run']['gputhresh'], shmem=slf._argflag['run']['shmem'], fstep=slf._argflag['chisq']['fstep'], limpar=slf._argflag['run']['limpar'])
...
#					ncpus=slf._argflag['run']['ncpus'], fstep=slf._argflag['chisq']['fstep'])
        ...
        if mr.status <= 0:
            ...
            msgs.info("Reason for convergence:"+msgs.newline()+utils.getreason(mr.status,verbose=slf._argflag['out']['verbose']),verbose=slf._argflag['out']['verbose'])
...
def sim_systematics(slf, p0new, parinfo, ntxt, edgearr):
    ...
    ms = alfit(p0new, parinfo=parinfo, functkw=fa,
                verbose=1, modpass=slf._modpass, miniter=slf._argflag['chisq']['miniter'], maxiter=slf._argflag['chisq']['maxiter'],
                ftol=slf._argflag['chisq']['ftol'], gtol=slf._argflag['chisq']['gtol'], xtol=slf._argflag['chisq']['xtol'],
                ncpus=slf._argflag['run']['ncpus'], ngpus=slf._argflag['run']['ngpus'], backend=slf._argflag['run']['backend'], gputhresh=slf._argflag['run']['gputhresh'], shmem=slf._argflag['run']['shmem'], fstep=slf._argflag['chisq']['fstep'])
    tend=time.time()
    if ms.status <= 0:
        if ms.status == -20:
        ...
        msgs.info("Reason for convergence:"+msgs.newline()+utils.getreason(ms.status,verbose=slf._argflag['out']['verbose']),verbose=slf._argflag['out']['verbose'])
    ...
    if slf._argflag['plot']['fits']:
        ...
        plot.plot_showall()
    # Get the results and print them to file
    save.save_model(slf, ms.params, ms.perror, [(tend - tstart)/3600.0, ms.fnorm, ms.dof, ms.niter, ms.status], printout=False, extratxt=[slf._argflag['sim']['dirname']+'/',".syst"+ntxt])
    return ms
