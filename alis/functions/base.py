...
# A tie label that is 'e' or 'E' followed by digits and nothing else. ALIS's
# parameter micro-syntax is "<value><tie label>", and the only exponent form it
# recognises is a *signed* one (E+04, e-3) -- so '1e4' does not read as 10000,
# it reads as the value 1.0 tied to a label 'e4'. See check_tie_label.
_AMBIGUOUS_EXPONENT = re.compile(r"^[eE][0-9]+$")


def check_tie_label(ival, tieval, idstr=""):
    """
    Reject a tie label that is almost certainly a mistyped exponent.

    A parameter token carries its value and its tie label in one word, so the
    parser splits '5.0da' into 5.0 and 'da'. An unsigned exponent is
    indistinguishable from that: '1e4' splits into 1.0 and a tie label 'e4',
    and two lines both written '1e4' silently share one free parameter instead
    of both being 10000. Nothing downstream can notice -- 'e4' is a perfectly
    legal label -- so it has to be caught here.

    Only 'e'/'E' followed by digits *and nothing else* is rejected, which is
    exactly the ambiguous case: a signed exponent (1.0E+04) has already been
    consumed by the caller and never reaches this function, and a label that
    merely starts that way ('E5t', 'e345j') cannot be a number and is left
    alone. A user who genuinely wants such a label renames it; a user who meant
    a number writes the sign.

    ival   : the whole parameter token as the user wrote it
    tieval : the tie label extracted from it
    idstr  : the model function's name, for the message

    Generated by RJC and Claude.
    """
    if _AMBIGUOUS_EXPONENT.match(tieval):
        msgs.error(
            "Ambiguous parameter '{0:s}'{1:s}".format(
                ival, " for the '" + idstr + "' model" if idstr else ""
            )
            + msgs.newline()
            + "ALIS reads this as the value {0:s} tied to a parameter labelled '{1:s}',".format(
                ival[: len(ival) - len(tieval)], tieval
            )
            + msgs.newline()
            + "not as scientific notation. If you meant a number, give the exponent"
            + msgs.newline()
            + "a sign ({0:s}E+{1:s} or {0:s}E-{1:s}); if you meant a tie label, rename it so".format(
                ival[: len(ival) - len(tieval)], tieval[1:]
            )
            + msgs.newline()
            + "it is not '{0:s}' followed only by digits.".format(tieval[0])
        )
...
class Base :
    ...
    CPU/GPU contract (Stage 4.1)
    ----------------------------
    Every model function implements ``call_CPU``. Implementing ``call_GPU`` is
    optional; a function opts in by *both* overriding ``call_GPU`` and setting
    the class attribute ``_gpu_supported = True``. The flag is required because
    several functions historically carried a copy of the inherited stub, so
    "is ``call_GPU`` overridden?" is not a reliable test of GPU capability.

    A function that does not opt in still works on a GPU run: the inherited
    ``call_GPU`` below falls back to ``call_CPU``, so the dispatcher (Task 4.3)
    can call either uniformly.

    ``call_GPU`` must reproduce ``call_CPU`` to an **absolute tolerance of
    1e-12** (stage doc Q4.3) and must use **float64 throughout** (Q4.6) -- a
    float32 path cannot meet that bar and would degrade the finite-difference
    Jacobian and hence the covariance matrix. It receives arrays that are
    **already on the device** and returns a device array: it launches a kernel,
    it does not transfer. Host<->device transfer, batching of same-type
    components, and the small-array CPU fallback all belong to the dispatcher
    (Task 4.3), not here.
    """

    # GPU opt-in flag; see the CPU/GPU contract above. Defined at class scope
    # (not in __init__) because subclasses set their attributes directly and do
    # not chain to Base.__init__, so an instance attribute would not reach them.
    ...
    def supports_gpu(self):
        """
        Return True if this function provides a real GPU implementation.

        Opting in requires overriding call_GPU *and* setting the class
        attribute _gpu_supported = True (see the CPU/GPU contract in the
        class docstring). Functions that have not opted in fall back to
        call_CPU, so a GPU run still produces correct results for them.

        Generated by RJC and Claude.
        """
        return bool(self._gpu_supported)

    def gpu_warmup_args(self):
        """
        Return (x, p, kwargs) for one throwaway call_GPU launch, or None.

        A cold GPU launch pays for a CUDA context and the @cuda.jit compile of
        the kernel -- on the Voigt, 0.58 s and 0.94 s against 0.25 ms of steady
        state. The 'run backend auto' probe (Task 4.3a) therefore warms every
        GPU worker before it times anything, and this is how it knows what to
        launch: gpu_dispatch.warm_up() finds every class with
        _gpu_supported = True and calls this method on it.

        A function that opts in to GPU support must implement it -- the
        arguments cannot be guessed, and a missing hook would silently leave
        that kernel to be compiled inside the timed Jacobian. Keep the arrays
        tiny; the point is to compile, not to compute.

        Returns
        -------
        tuple or None
            ``(x, p, kwargs)``: a small host wavelength array, a small host
            parameter array (one row is enough), and any keyword arguments
            call_GPU needs (typically ``{"ae": ..., "mkey": ...}``). The
            dispatcher moves x and p to the device. ``None`` means "nothing to
            warm", which is only correct for a function with no GPU path.
    ...
    def call_GPU(self, x, p, ae='em', mkey=None, ncpus=1):
        ...
        --------------------------------------------------------
        x  : device array of wavelengths (already on the GPU)
        p  : device array of parameters for this model, one row per
             component, so a single launch covers a batch of profiles
        ae : 'em' (sum components) / 'ab' (product) / 'zl' / 'cv',
             exactly as for call_CPU
        --------------------------------------------------------
        Returns a device array of the same length as x.

        This default is the graceful "no GPU implementation" fallback: it
        evaluates on the CPU and returns the result, so a function that has
        not been ported still works under the GPU backend. The signature
        mirrors call_CPU precisely so the two are interchangeable for the
        dispatcher (Task 4.3).

        An overriding implementation must be numerically equivalent to
        call_CPU to 1e-12 absolute, in float64 -- see the class docstring.
    ...
    def load(self, instr, cntr, mp, specid):
        ...
        def check_tied_param(ival, cntr, mps, iind):
            ...
            check_tie_label(ival, tieval, self._idstr)
