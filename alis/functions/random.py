...
class Random(base.Base):
    ...
    def __init__(self, prgname="", getinst=False, atomic=None, verbose=2):
        ...
        self._keywd   = dict({'specid':[], 'continuum':False, 'blind':False, 'command':"random()", 'start':True, 'blindseed':0,  'blindrange':[]})		# Additional arguments to describe the model --- 'input' cannot be used as a keyword
        self._keych   = dict({'specid':0,  'continuum':0,     'blind':0,     'command':1,          'start':0,    'blindseed':0,  'blindrange':0})		# Require keywd to be changed (1 for yes, 0 for no)
        self._keyfm   = dict({'specid':"", 'continuum':"",    'blind':"",    'command':"",         'start':"",   'blindseed':"", 'blindrange':""})	# Format for the keyword. "" is the Default setting
    ...
    def parin(self, i, par, parb, start=False):
        ...
        if self._keywd['start']:
            if i == 0:
                ...
                exec("pin = numpy.random."+parb["cmd"], namespace)
                pin = namespace['pin']
