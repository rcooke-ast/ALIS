...
try:
    ...
    # Bound to None rather than left undefined: without this, using the 'lsf'
    # function on a machine that has no linetools fails with a bare
    # "NameError: name 'ltLSF' is not defined" from deep inside the model
    # evaluation, which says nothing about what is actually missing.
    # linetools is an optional dependency -- only this function needs it.
    ltLSF = None


def _require_linetools():
    """
    Abort with a useful message if linetools is not installed.

    Generated by RJC and Claude.
    """
    if ltLSF is None:
        msgs.error("The 'lsf' model function needs linetools, which is not"
                   + msgs.newline() + "installed. Install it with: pip install linetools")
