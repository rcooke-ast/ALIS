...
class VoigtConv(base.Base) :
    ...
    def __init__(self, prgname="", getinst=False, atomic=None, verbose=2):
        self._idstr   = 'voigtconv'			# ID string for this class
        ...
        self._parid   = ['sigma', 'damping']		# Name of each parameter
        ...
        self._limited = [ [1  ,0  ], [1  ,0  ] ]	# Should any of these parameters be limited from below or above
        self._limits  = [ [0.0,0.0], [0.0,0.0] ]	# What should these limiting values be
        self._svfmt   = [ "{0:.3g}", "{0:.3g}" ]	# Specify the format used to print or save output
    ...
    def call_CPU(self, x, y, p, ncpus=1, mkey=None):
        ...
        if cond:
            ysize=y.size
            fsigd=20.0*sigd  # Make this large, so it includes the wings
            ...
            if np.size(sigd) == 1:
                ...
                wv = x[df] * 1.0e-8
                bl = sigd * wv
                a = damping * 1.0E8 * wv * wv / (3.76730313461770655E11 * bl)
                ww = x * 1.0e-8
