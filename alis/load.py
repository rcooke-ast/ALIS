...
def set_params(lines, argflag, setstr="", verbose=None):
    ...
    Settings are applied in file order, so a setting given twice in the same
    block silently takes its *last* value. That is correct behaviour -- it is
    how the model file overrides 'settings.alis' -- but within one block it is
    almost always a mistake, and a silent one: a value added at the top of a
    file to override something further down has no effect at all. Repeats
    within a single call are therefore warned about (repeats *across* calls are
    the intended override mechanism and are not).
    """
    seen = {}
...
def call_function_load(funccall, funcinst, instr, cntr, modpass, specid, **kwargs):
    """
    Call a model function's ``load()`` with a per-line record of which
    parameters the user actually named.

    Since Stage 2 each function is instantiated once and reused for every
    line of that type. ``load()`` records the parameters named on a line in
    the *instance's* ``self._keywd['input']`` and then shallow-copies
    ``_keywd`` into ``modpass['mkey']``, so two things leak: the record is
    never cleared between lines, and every line ends up sharing one ``input``
    dict. ``parout`` reads that record to decide what to echo, so it printed,
    on every line of a function, the union of the parameters named on *any*
    line of it.

    That is the Stage 5.4 ``damping=`` bug: ``helium34`` names ``damping`` on
    1 of 12 voigt lines, and all 12 were written out with it. Because voigt's
    damping defaults to fixed but a written-out ``damping=0.0000000`` carries
    no fix marker, re-reading the saved model turned 11 fixed parameters into
    free ones and changed the DOF.

    Giving the instance a fresh ``input`` dict per call fixes both halves at
    once: it clears the record, and it leaves the previous line's ``mkey``
    entry holding the only reference to its own snapshot.

    Generated by RJC and Claude.
    """
    funcinst._keywd['input'] = dict.fromkeys(funcinst._keywd['input'], 0)
...
DEFAULT_ATOMIC = "atomic.ecsv"


def atomic_datadir():
    """
    The directory holding the shipped data files (``alis/data/``).

    Derived from this module's own location. It used to be reconstructed by
    splitting ``argflag['run']['prognm']`` on ``'/'`` and re-joining everything
    before the last separator, which is path-separator-specific and made
    ``prognm`` look as though it had to name a real file when only its
    directory was ever used (Stage 5.2).
...
def read_atomic_table(fname, verbose=2):
    """
    Read an atomic-data file into an ``astropy`` table.

    ECSV is the format ALIS ships and the one to use. VOTable is still read so
    that a user's own ``atomic.xml`` keeps working, but it is deprecated and
    warns (RJC, Q5.17). The choice is made on the file extension.
    ...
    if fname.lower().endswith(".xml"):
        ...
        msgs.warn("Atomic data in the VOTable (.xml) format is deprecated -"
                  + msgs.newline() + fname + msgs.newline()
                  + "Convert it with alis/data/convert_xmlFormat_to_ecsvFormat.py,"
                  + msgs.newline()
                  + "or ask RJC to add the transitions to the shipped atomic.ecsv",
                  verbose=verbose)
        try:
            return parse_single_table(fname).to_table()
        except IOError:
            msgs.error("The filename does not exist -" + msgs.newline() + fname)
    try:
        return Table.read(fname, format="ascii.ecsv")
    except IOError:
        msgs.error("The filename does not exist -" + msgs.newline() + fname)


ATOMIC_COLUMNS = ['MassNumber', 'AtomicMass', 'Element', 'Ion', 'RestWave',
                  'fval', 'Gamma', 'q', 'K']


def check_atomic_table(table, fname):
    """
    Fail early and legibly if an atomic-data file is missing a column.

    Without this the absence shows up much later, as a KeyError inside the
    profile calculation.
    ...
    if missing:
        msgs.error("Atomic data file is missing the column(s) {0:s} -".format(
            ", ".join(missing)) + msgs.newline() + fname)


def duplicate_transitions(table):
    """
    Transitions listed under more than one ``MassNumber``.

    Keyed on Element+Ion+RestWave *ignoring* MassNumber -- the check that would
    have caught the radioactive isotopes mislabelled in the candidate atomic
    files (Q5.8), where the same line was entered twice under different mass
    numbers so it was ambiguous which isotope a component should use.

    Returns a list of ``(element, ion, restwave, [mass numbers])``. It reports
    and never modifies: genuine isotope structure looks identical to a
    mislabelling here, and only a person can tell them apart. The shipped
    ``atomic.ecsv`` legitimately has four such groups (6/7Li, 12/13C,
    54/56/57/58Fe), which is why this is run when a file is *converted* rather
    than on every load -- a warning every fit would train users to ignore it.

    Generated by RJC and Claude.
    """
    groups = {}
    for i in range(len(table)):
        key = (str(table['Element'][i]).strip(), str(table['Ion'][i]).strip(),
               float(table['RestWave'][i]))
        groups.setdefault(key, []).append(int(table['MassNumber'][i]))
    return [(k[0], k[1], k[2], sorted(set(v)))
            for k, v in groups.items() if len(set(v)) > 1]
...
def load_atomic(slf):
    ...
    datadir = atomic_datadir() + os.sep
    # If the user specifies the atomic data file, make sure that it exists
    if os.path.exists(atmname):
        fname = atmname
    elif os.path.exists(datadir+atmname):
        msgs.info("Using atomic datafile:"+msgs.newline()+datadir+atmname, verbose=slf._argflag['out']['verbose'])
        fname = datadir+atmname
    else:
        msgs.warn("Couldn't find atomic datafile:"+msgs.newline()+atmname, verbose=slf._argflag['out']['verbose'])
        msgs.warn("Also tried atomic datafile:"+msgs.newline()+datadir+atmname, verbose=slf._argflag['out']['verbose'])
        msgs.info("Using atomic datafile:"+msgs.newline()+datadir+DEFAULT_ATOMIC, verbose=slf._argflag['out']['verbose'])
        fname = datadir+DEFAULT_ATOMIC
    table = read_atomic_table(fname, verbose=slf._argflag['out']['verbose'])
    check_atomic_table(table, fname)
    ...
    isotope = np.char.add(np.asarray(table['MassNumber']).astype('U'), np.asarray(table['Element']).astype('U'))
    ...
    atmdata['Ion'] = np.char.add(np.char.add(isotope, "_").astype('U'), np.asarray(table['Ion']).astype('U'))
    atmdata['Wavelength'] = np.array(table['RestWave'])
    atmdata['fvalue'] = np.array(table['fval'])
    atmdata['Gamma'] = np.array(table['Gamma'])
    atmdata['Qvalue'] = np.array(table['q'])
    atmdata['Kvalue'] = np.array(table['K'])
    ...
    mass_of = {}
    warned = set()
    for iso, mass in zip(isotope, np.asarray(table['AtomicMass'])):
...
def load_data(slf, datlines, data=None):
    ...
    keywords = ['specid','fitrange','loadrange','bufferpix','systematics','systmodule','resolution','shift','columns','plotone','nsubpix','bintype','loadall','label','yrange']
...
#								if defn not in dir(usrmod): msgs.error("Systematics module {0:s} must contain function {1:s}".format(module,defn))
    ...
    for i in range(0,len(datlines)):
        ...
        bufferpix = None  # (left, right) pixel buffer, if the user set one
        ...
        if len(linspl) > 1:
            for j in range(1,len(linspl)):
                ...
                if   kwdspl[0] == 'specid':
                ...
                elif kwdspl[0] == 'bufferpix':
                    # Number of pixels to load either side of the fitrange
                    # (Stage 5.4). Accepts one value for a symmetric buffer or
                    # [left,right]; the writer always emits the two-sided form,
                    # since the resolution rule it replaces does not cover the
                    # same number of pixels on both sides.
                    bufspl = kwdspl[1].strip('[]()').split(',')
                    try:
                        if len(bufspl) == 1:
                            bufferpix = (int(bufspl[0]), int(bufspl[0]))
                        elif len(bufspl) == 2:
                            bufferpix = (int(bufspl[0]), int(bufspl[1]))
                        else:
                            raise ValueError
                    except ValueError:
                        msgs.error("read data 'bufferpix' takes one integer or [left,right] on line -"+msgs.newline()+datlines[i])
                    if bufferpix[0] < 0 or bufferpix[1] < 0:
                        msgs.error("read data 'bufferpix' cannot be negative on line -"+msgs.newline()+datlines[i])
                elif kwdspl[0] == 'systematics':
                    ...
                    for k in range(len(colspl)):
                        ...
                        if len(clspid)==2: wfe[clspid[0].strip()] = int(clspid[1].strip())
                    ...
                    if len(yrngspl) == 2:
                        if yrngspl[0].lower() == "none":
                            ...
                            yrngmin = float(yrngspl[0])
                        if yrngspl[1].lower() == "none":
                            yrngmax = None
                        else:
                            yrngmax = float(yrngspl[1])
                    elif yrngspl == ['all']:
...
#					else: nsr = 1.0/slf._argflag['generate']['peaksnr']
        ...
        if lwavemin == -1.0:
            ...
            if bufferpix is not None:
                # An explicit pixel buffer overrides the resolution rule
                # (Stage 5.4). This is what makes a saved .mod.out reload the
                # same pixels: the resolution written into it is the *fitted*
                # one, so a buffer derived from it would differ from the buffer
                # the original run used. A pixel count does not move.
                wfit = np.where((wavein >= lwavemin) & (wavein <= lwavemax))[0]
                if wfit.size == 0:
                    msgs.error("No data was found within the fitrange limits for the file -"+msgs.newline()+filename)
                ilo = max(0, wfit[0] - bufferpix[0])
                ihi = min(wavein.size - 1, wfit[-1] + bufferpix[1])
                if wavein[ilo] > wvmnres or wavein[ihi] < wvmxres:
                    msgs.warn("The 'bufferpix' buffer is narrower than the instrumental"+msgs.newline()+
                              "resolution needs for the file -"+msgs.newline()+filename+msgs.newline()+
                              "The convolution may be truncated near the edges of the fitrange."+msgs.newline()+
                              "Increase 'bufferpix' or remove it to use the resolution width.",
                              verbose=slf._argflag['out']['verbose'])
                wvmnres, wvmxres = wavein[ilo], wavein[ihi]
...
def load_userdata(data, colspl, wfe, verbose=2):
    ...
    for j in range(len(wfek)):
        ...
        uccnt+=1
    datain = data[usecols,:]
    wavein, fluxin, fluein = datain[ucind['wave'],:].astype(float), datain[ucind['flux'],:].astype(float), datain[ucind['error'],:].astype(float)
    ...
    if wfe['fitrange']  != -1:
        try:
            ...
            fitrin = np.ones(wavein.size).astype(int)
    else: fitrin = np.ones(wavein.size).astype(int)
    ...
    if wfe['loadrange']  != -1:
        try:
            ...
            loadin = np.ones(wavein.size).astype(int)
    else: loadin = np.ones(wavein.size).astype(int)
    # Now return
...
#		uccnt+=1
#	try:
#		datain = np.loadtxt(filename, dtype=float, usecols=usecols).transpose()
#	except:
    wavein, fluxin, fluein, contin, zeroin, systin, fitrin, loadin = None, None, None, None, None, None, None, None
    for j in range(len(wfek)):
        if wfe[wfek[j]] == -1: continue
        try:
            onecol = np.loadtxt(filename, dtype=float, usecols=(wfe[wfek[j]],))
    ...
    if fitrin is None: fitrin = np.ones(wavein.size).astype(int)
    if loadin is None: loadin = np.ones(wavein.size).astype(int)
    # Now return
    return wavein, fluxin, fluein, contin, zeroin, systin, fitrin, loadin
...
def load_fits(filename, colspl, wfe, verbose=2, ext=0, datatype='default'):
    ...
    else: zeroin = np.zeros(wavein.size).astype(float)
    ...
    if wfe['systematics'] != -1:
        try:
            ...
            systin = np.zeros(wavein.size).astype(float)
    else: systin = np.zeros(wavein.size).astype(float)
    # Read in the fit range
    if wfe['fitrange']  != -1:
        try:
            fitrin = datain[wfe['fitrange'],:].astype(float)
        except:
            ...
            fitrin = np.ones(wavein.size).astype(float)
    else: fitrin = np.ones(wavein.size).astype(float)
    # Read in the load range
    if wfe['loadrange']  != -1:
        try:
            loadin = datain[wfe['loadrange'],:].astype(float)
        except:
            ...
            loadin = np.ones(wavein.size).astype(float)
    else: loadin = np.ones(wavein.size).astype(float)
    # Now return
    return wavein, fluxin, fluein, contin, zeroin, systin, fitrin, loadin
...
def load_model(slf, modlines, updateself=True):
    ...
    for i in range(len(modlines)):
        ...
        if mdlspl[0].strip() == 'emission':
            ...
            modpass, paridt = call_function_load(slf._funcarray[1][mdlspl[0]], slf._funcarray[2][mdlspl[0]], instr, cntr, modpass, slf._specid)
    ...
    for i in range(len(slf._resn)):
        for j in range(len(slf._resn[i])):
            ...
            modpass, paridt = call_function_load(slf._funcarray[1][fspl[0]], slf._funcarray[2][fspl[0]], fspl[1], cntr, modpass, slf._specid)
    ...
    for i in range(len(slf._shft)):
        for j in range(len(slf._shft[i])):
            if slf._shft[i][j] == "none":
                ...
                modpass, paridt = call_function_load(slf._funcarray[1][fspln], slf._funcarray[2][fspln], fsplv, cntr, modpass, slf._specid, forcefix=True)
                ...
                for j in range(2):
                    try:
                        if limspl[j].lower() == 'none':
                            ...
                            slf._funcarray[2][mdlspl[1]]._limits[find][j]  = float(limspl[j])
                    except:
...
#			wvrng = [slf._wavefull[sp][ll:lu].min(),slf._wavefull[sp][ll:lu].max()]
    ...
    for sp in range(len(slf._posnfull)):
        for sn in range(len(slf._posnfull[sp])-1):
            ...
            binlen = 1.0/float(nexbins[sp][sn])
...
def check_par_influence(slf, parin, refpinfl, verbose=2):
    """Warn if the parameter influence changed from its start-of-fit value.

    The influence table (which parameters affect each spectrum/snip) is fixed at
    the start of the fit and must not change during minimisation. If it differs
    when recomputed at the best-fit parameters ``parin``, a component has moved
    across a region boundary -- something has gone wrong. Compares against the
    reference ``refpinfl`` (the start-of-fit ``[opinfl, pinfl]``) and warns per
    region that differs. Returns True if the influence is stable.

    Generated by RJC and Claude.
    """
    try:
        newpinfl = load_par_influence(slf, parin)
    except Exception as err:
        msgs.warn("Could not re-check the parameter influence: {0}".format(err),
                  verbose=verbose)
        return True
    changed = pinfl_changed(refpinfl, newpinfl)
    for sp, sn in changed:
        msgs.warn("Parameter influence changed during the fit for "
                  "specid={0:s} (region {1:d}) -- a model component may have "
                  "moved across a region boundary; the fit result should be "
                  "treated with caution".format(str(slf._specid[sp]), sn),
                  verbose=verbose)
    if not changed:
        msgs.info("Parameter influence is stable (unchanged from the start of "
                  "the fit).", verbose=verbose)
    return not changed


def pinfl_changed(refpinfl, newpinfl):
    """Return the list of ``(sp, sn)`` whose parameter-influence set differs.

    Compares the global parameter-id lists (``pinfl`` = index [1] of the
    ``[opinfl, pinfl]`` influence structure) region by region, as sets. Pure and
    testable in isolation (no model stack needed).

    Generated by RJC and Claude.
    """
    ref, new = refpinfl[1], newpinfl[1]
    changed = []
    for sp in range(len(new)):
        for sn in range(len(new[sp])):
            if set(np.asarray(ref[sp][sn]).tolist()) != \
                    set(np.asarray(new[sp][sn]).tolist()):
                changed.append((sp, sn))
    return changed
