...
class Random(alfunc_base.Base):
    ...
    def __init__(self, prgname="", getinst=False, atomic=None, verbose=2):
        ...
        self._keywd   = dict({'specid':[], 'continuum':False, 'blind':False, 'command':"random()", 'start':True})		# Additional arguments to describe the model --- 'input' cannot be used as a keyword
        self._keych   = dict({'specid':0,  'continuum':0,     'blind':0,     'command':1,          'start':0})		# Require keywd to be changed (1 for yes, 0 for no)
        self._keyfm   = dict({'specid':"", 'continuum':"",    'blind':"",    'command':"",         'start':""})	# Format for the keyword. "" is the Default setting
