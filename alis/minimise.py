...
def _make_gpu_pool(ngpus, threshold, verbose):
    ...
    return ctx.Pool(processes=ngpus, initializer=_gpu_worker_init,
                    initargs=(counter, threshold, verbose))
...
class alfit(object):
    ...
    def __init__(self, xall=None, functkw={}, funcarray=[None, None, None], parinfo=None,
                 ...
                 gputhresh=None, fstep=1.0, debug=0, convtest=False):
        """
  Inputs:
    fcn:
   ...
   Outputs:
     ...
     .perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        # Parallel backend (Stage 4.3): CPU or GPU for the whole fit, never both
        # (Q4.8). Resolved here, before the first model evaluation, because the
        # base call in prepare_iteration() below already goes through the
        # dispatcher. An unusable GPU degrades to the CPU with a warning.
        self.verbose = verbose
        self.gputhresh = gputhresh
        self.backend, self.ngpus = gpu.resolve_backend(ngpus, verbose=verbose)
        if self.backend == "gpu":
    ...
    def fdjac2(self, x, fvec, step=None, ulimited=None, ulimit=None, dside=None,
        ...
        if self._pool is None:
            if self.backend == "gpu":
                ...
                # No Pool initializer: on 'spawn' an initializer blocks Pool
                # creation until every worker finishes its (heavy) re-import;
                # here the workers start lazily and the constant state travels
                # per chunk.
                self._pool = mpPool(processes=self.ncpus)
        ...
        payload = [(state, fvec, xall,
                   ...
                   for chunk in chunks]
        for chunk_res in self._pool.map(_worker_chunk, payload):
