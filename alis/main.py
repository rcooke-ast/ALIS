...
def build_funcarray(argflag, atomic):
    ...
    instances = base.call(getinst=True,
...
class ClassMain:
    ...
    def __init__(self, argflag, getinst=False, modelfile=None, parlines=[], datlines=[], modlines=[], lnklines=[], data=None, fitonly=False, verbose=None, cli_overrides=None):
        ...
        # changed -- re-applied after the model file below, and recorded in the
        # .mod.out by save_model (Stage 6.1, Q6.12/Q6.20).
        ...
        if modelfile is not None:
            ...
            self._parlines, self._datlines, self._modlines, self._lnklines = load.load_input(self)
        # The command line beats the model file (Stage 6.1, Q6.12). load_input
        # has just applied the file's own 'par' lines over everything, which
        # used to silently undo an explicit flag -- 'run_alis -p 0 fit.mod' ran
        ...
        if self._argflag['plot']['only']:
            ...
            while not complete:
                ...
                m = alfit(self._modpass['p0'], parinfo=parinfo, functkw=fa, funcarray=self._funcarray,
                        ...
                        ncpus=self._argflag['run']['ncpus'], ngpus=self._argflag['run']['ngpus'], backend=self._argflag['run']['backend'], gputhresh=self._argflag['run']['gputhresh'], shmem=self._argflag['run']['shmem'], fstep=self._argflag['chisq']['fstep'],limpar=self._argflag['run']['limpar'])
                ...
                if m.status == -20:
                    ...
                    while True:
                        ...
                        mc = alfit(mpars.params, parinfo=parinfo, functkw=fa, funcarray=self._funcarray,
                                verbose=self._argflag['out']['verbose'], modpass=self._modpass, miniter=self._argflag['chisq']['miniter'], maxiter=self._argflag['chisq']['maxiter'],
                                atol=self._argflag['chisq']['atol'], ftol=self._argflag['chisq']['ftol'], gtol=self._argflag['chisq']['gtol'], xtol=self._argflag['chisq']['xtol'],
                                ncpus=self._argflag['run']['ncpus'], ngpus=self._argflag['run']['ngpus'], backend=self._argflag['run']['backend'], gputhresh=self._argflag['run']['gputhresh'], shmem=self._argflag['run']['shmem'], fstep=self._argflag['chisq']['fstep'],limpar=self._argflag['run']['limpar'], convtest=True)
                        # Update parameters
...
#				plot.plot_showall()
            ...
            if self._argflag['sim']['random'] is not None:
                from alis import simulate
            ...
            elif self._argflag['sim']['perturb'] is not None:
                from alis import simulate
                if m.perror is None or m.perror is m.params: msgs.warn("Fitting routine interrupted. Cannot perform simulations",verbose=self._argflag['out']['verbose'])
                else:
                    msgs.info("Starting simulations",verbose=self._argflag['out']['verbose'])
