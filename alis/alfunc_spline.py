...
class Spline(alfunc_base.Base) :
    """
    Returns a spline flux values given by p[0], p[1], p[2], ... and locations given by 'locations'.
    If locations is not specified, the locations will be linearly sampled between the minimum and
    maximum wavelengths. These are provided as input, or determined from the loadrange parameter.
    """
    def __init__(self, prgname="", getinst=False, atomic=None, verbose=2):
        self._idstr   = 'spline'								# ID string for this class
        self._pnumr   = 1											# Total number of parameters fed in
        self._keywd   = dict({'specid':[], 'continuum':False, 'blind':False, 'k':3,  'wavemin':-1.0, 'wavemax':-1.0, 'locations':[], 'blindseed':0,  'blindrange':[]})			# Additional arguments to describe the model --- 'input' cannot be used as a keyword
        self._keych   = dict({'specid':0,  'continuum':0,     'blind':0,     'k':0,  'wavemin':0,    'wavemax':0,    'locations':0, 'blindseed':0,  'blindrange':0})			# Require keywd to be changed (1 for yes, 0 for no)
        self._keyfm   = dict({'specid':"", 'continuum':"",    'blind':"",    'k':"{1:.8g}", 'wavemin':"{1:.8g}",   'wavemax':"{1:.8g}",   'locations':"", 'blindseed':"",  'blindrange':""})		# Format for the keyword. "" is the Default setting
        self._parid   = ['flux']				# Name of each parameter
        ...
        self._prekw   = ['k', 'wavemin', 'wavemax']							# Specify the keywords to print out before the parameters
        ...
        def model(par, karr):
            ...
            if len(karr['locations']) == 0:
                ...
                if karr['wavemax'] > 0.0:
                    ...
                    xmax = np.max(x)
                xspl = np.linspace(xmin, xmax, len(par))
            elif len(karr['locations']) == len(par):
                xspl = karr['locations']
            else:
                msgs.error("'locations' parameter must have same length as input parameters in spline function")
            #pdb.set_trace()
            tck = splrep(xspl, par, k=karr['k'])
            modret = splev(x, tck)
            return modret
        #############
