...
carries the fit configuration/data the evaluation reads and writes.
...
@dataclass
class FitState:
    """Explicit, picklable state for model evaluation (Stage 2.4).

    Holds exactly the attributes the evaluation path (``myfunct`` ->
    ``model_func`` -> ``load.load_par_influence`` / ``load.load_subpixels``)
    reads or writes. It replaces ``functkw['fdict'] = self.__dict__`` +
    ``myfunct_wrap``'s ``ClassMain`` reconstitution: the minimiser is handed a
    ``FitState`` and calls the standalone functions directly. Fields mirror the
    orchestrator's ``_``-prefixed attributes (shared references, not copies),
    so :func:`_minimiser_eval`'s per-call ``copy.copy`` reproduces the old
    reconstitution semantics exactly (local rebinds, shared mutations).

    Generated by RJC and Claude.
    """

    _argflag: Any = None
    _chisq_init: Any = None
    _contfinal: Any = None
    _contfull: Any = None
    _contspx: Any = None
    _datopt: Any = None
    ...
    _levadd: Any = None
    _links: Any = None
    _modconv_all: Any = None
    _modfinal: Any = None
    # Stage 4.7: the per-snip component plan, resolved once per fit and cached
    # here. Deliberately not read from the orchestrator (which has no such
    # attribute), so a reloaded model gets a fresh FitState and a fresh plan.
    _mplan: Any = None
    _modpass: Any = None
    # Stage 4.8: (x, per-snip fitted-pixel counts), cached the same way and for
    ...
    @classmethod
    def from_orchestrator(cls, orch):
        """Snapshot the state the evaluation needs from ``orch`` (ClassMain).

        Field values are shared references, exactly as ``fdict =
        self.__dict__`` was.

        Generated by RJC and Claude.
        """
        return cls(**{f.name: getattr(orch, f.name, None)
                      for f in fields(cls)})


def _minimiser_eval(p, fjac=None, x=None, y=None, err=None, state=None,
                    ddpid=None, pp=None, getemab=False, emab=None, output=0):
    """Minimiser entry point (replaces ``myfunct_wrap``).

    Evaluates the model at ``p`` and returns the residual/model. Stage 4.0
    dropped the former per-call ``copy.copy(state)``: with ``_pinfl`` frozen (a
    per-fit constant, no longer rebuilt during the fit) the evaluation's only
    remaining writes to ``state`` are per-call scratch (``_modfinal`` /
    ``_contfinal`` / ``_zerofinal``, rebuilt from scratch each call) that no
    fit-path caller reads back, and in the workers ``state`` is already a private
    unpickled copy -- so no isolation copy is needed. This makes the derivative a
    pure function of ``(per-iteration invariants, perturbed params)`` returning
    its residual column, the shape the GPU kernel path needs.

    Generated by RJC and Claude.
    """
    return myfunct(p, fjac=fjac, x=x, y=y, err=err, state=state,
                   ddpid=ddpid, pp=pp, getemab=getemab, emab=emab,
                   output=output)


class SnipPlan:
    """Which model components apply to one (sp, sn), resolved once per fit.

    Deciding that used to be redone inside every model evaluation: for each of
    the 351 DH_orders snips, every one of the ~245 model components was tested
    against ``emab`` and ``specid``, the emission/absorption grouping was
    rebuilt, and the model-type list was searched and grown with
    ``np.where``/``np.append``. None of it depends on the parameters -- it
    follows from ``_modpass`` and ``_specid``, both fixed for the whole fit --
    and it was measured at **41.6% of a derivative column** on DH_orders, which
    is what Stage 4.7 removes.

    Attributes
    ----------
    zl : list of int
        Component indices carrying the zero level (only reached at ``sn == 0``).
    entries : list of tuple
        ``(i, mtyp, ea, mid)`` in evaluation order: the component index, its
        model type, which emission/absorption group it belongs to, and its slot
        within that group. Consumed only when the snip is influenced.
    slots : list of list of str
        Per group, the model types in first-seen order -- what the old code
        accumulated into ``modtyp[sp][sn][ea]``.

    Generated by RJC and Claude.
    """

    __slots__ = ("zl", "entries", "slots")

    def __init__(self, zl, entries, slots):
        self.zl = zl
    ...
    @property
    def nea(self):
        """Number of emission/absorption groups in this snip."""
        return len(self.slots)


def build_model_plan(state, pos):
    """Resolve the per-snip component structure for the whole fit (Stage 4.7).

    Mirrors the filtering and grouping the setup loop of :func:`model_func`
    used to perform on every call, in the same order, so the arrays it feeds
    come out identical. The model-validity errors it can raise are the same
    ones -- they now surface at the first evaluation rather than at each.

    Generated by RJC and Claude.
    """
    emab = state._modpass['emab']
    mtyps = state._modpass['mtyp']
    mkeys = state._modpass['mkey']
    ncomp = len(mtyps)
    plans = []
    for sp in range(len(pos)):
        specid = state._specid[sp]
        lastemab = ['' for all in pos[sp][:-1]]
        iea = [-1 for all in pos[sp][:-1]]
        plans.append([])
        for sn in range(len(pos[sp]) - 1):
            zl, entries, slots = [], [], []
            for i in range(ncomp):
                if emab[i] in ['cv', 'sh']: continue
                if specid not in mkeys[i]['specid']: continue
                if emab[i] == 'zl':
                    if sn != 0: continue
                    zl.append(i)
                    continue
                if lastemab[sn] == '' and emab[i] != 'em':
                    if emab[i] != 'va':
                        msgs.error("Model for specid={0:s} must specify emission before absorption".format(state._snipid[sp]))
                if lastemab[sn] != emab[i] and emab[i] != 'va':
                    slots.append([])
                    iea[sn] += 1
                    lastemab[sn] = emab[i]
                mtyp = mtyps[i]
                ea = iea[sn]
                if mtyp not in slots[ea]:
                    slots[ea].append(mtyp)
                entries.append((i, mtyp, ea, slots[ea].index(mtyp)))
            plans[sp].append(SnipPlan(zl, entries, slots))
    return plans


def model_plan(state, pos):
    """The cached :func:`build_model_plan` for this fit, built on first use.

    Rebuilt if the snip layout does not match, so a caller that evaluates a
    different set of positions -- or a model reloaded by the ``iterate`` path
    -- cannot be served a stale plan. ``getattr``/``setattr`` rather than the
    field directly, because ``state`` is a ``ClassMain`` on the plotting and
    simulation paths, not only a :class:`FitState`.

    Generated by RJC and Claude.
    """
    plan = getattr(state, "_mplan", None)
    if plan is not None and len(plan) == len(pos):
        if all(len(plan[sp]) == len(pos[sp]) - 1 for sp in range(len(pos))):
            return plan
    plan = build_model_plan(state, pos)
    state._mplan = plan
    return plan


def build_fit_windows(state, x, pos):
    """Fitted-pixel count of each snip (Stage 4.8).

    The convolution loop skips a snip a derivative does not influence, but it
    still had to know how many fitted pixels that snip contributes so it could
    advance its position in the packed model vector -- and it worked that out
    with a ``np.where`` and a ``np.isin`` over the snip's wavelengths, every
    time. The count depends only on ``x``, ``_posnfit`` and ``_wavefit``, all
    fixed for the fit, so on DH_orders that was ~145,000 ``np.isin`` calls per
    Jacobian for 351 numbers.
    ...
    for sp in range(len(pos)):
        ...
        for sn in range(len(pos[sp]) - 1):
            ...
            wB = np.where(wA == True)
            sizes[sp].append(x[sp][ll:lu][w][wB].size)
    return sizes


def fit_windows(state, x, pos):
    """The cached :func:`build_fit_windows`, built on first use.

    Keyed on the identity of ``x`` as well as the snip layout: unlike the
    component plan these counts depend on the wavelength array, and
    ``ClassMain.model_func`` lets a caller pass a different one.

    Generated by RJC and Claude.
    """
    cached = getattr(state, "_nfitpix", None)
    if cached is not None and cached[0] is x and len(cached[1]) == len(pos):
        if all(len(cached[1][sp]) == len(pos[sp]) - 1 for sp in range(len(pos))):
            return cached[1]
    sizes = build_fit_windows(state, x, pos)
    state._nfitpix = (x, sizes)
    return sizes
...
def model_func(state, x, p, pos, ddpid=None, getemab=False, output=0, compcache=None):
    ...
    # discarded on the per-call copy.copy(state), and once that copy is dropped
    # (Task 4.0) it must not mutate the shared influence table. Callers set _pinfl
    # explicitly before evaluating (main.load_par_influence for the fit; the sim
    # and iterate paths set it after loading their model).
#		print ddpid, state._pinfl
#		if ddpid is not None and state._qanytied:
#			p = load.load_tied(p, state._ptied, infl=state._pinfl)
...
#		modgpu=[]
    ...
    state._modfinal, state._contfinal, state._zerofinal = [], [], []
...
#		print state._snipid
#		print state._specid
    plan = model_plan(state, pos)
    shind = np.where(np.array(state._modpass['emab']) == 'sh')[0][0]
...
#			mcont.append(contspx[sp])
        ...
        state._modfinal.append(-9.999999999E9*np.ones(x[sp].size))
        state._contfinal.append(-9.999999999E9*np.ones(x[sp].size))
        state._zerofinal.append(-9.999999999E9*np.ones(x[sp].size))
        for sn in range(len(pos[sp])-1):
            ll = pos[sp][sn]
            lu = pos[sp][sn+1]
#				w = np.where((x[sp][ll:lu] >= state._posnfit[sp][2*sn+0]) & (x[sp][ll:lu] <= state._posnfit[sp][2*sn+1]))
#				if sn == 0: state._modfinal.append(-1.0*np.ones(np.size(w)))
#				else: state._modfinal[sp] = np.append(state._modfinal[sp], -1.0*np.ones(np.size(w)))
            # Calculate the spectrum shift
            shmtyp = state._modpass['mtyp'][shind]
            state._funcarray[2][shmtyp]._keywd = state._modpass['mkey'][shind]
            shparams = state._funcarray[1][shmtyp].set_vars(state._funcarray[2][shmtyp], p, state._levadd[shind], state._modpass, shind)
            wvrngt = state._funcarray[1][shmtyp].call_CPU(state._funcarray[2][shmtyp], x[sp][ll:lu], shparams)
            shind += 1
            wvrng = [wvrngt.min(),wvrngt.max()]
            # Stage 4.7: which components apply here, and how they group, is a
            # property of the model and the data -- not of the parameters -- so
            # it is resolved once per fit (build_model_plan) instead of being
            # re-derived, with ~245 emab/specid tests and a growing numpy
            # string array, on every one of the 413 derivative columns.
            snip = plan[sp][sn]
            # The zero level is read before the influence test, exactly as the
            # loop it replaces did: an un-influenced snip still sets it.
            for i in snip.zl:
                if len(zerlev[sp]) != 0:
                ...
                params = state._funcarray[1][mtyp].set_vars(state._funcarray[2][mtyp], p, state._levadd[i], state._modpass, i, spid=state._specid[sp], levid=state._levadd)
                if len(params) == 0: continue
                zerlev[sp].append(mtyp)
                zerlev[sp].append(np.array([params]))
                zerlev[sp].append(state._modpass['mkey'][i])
            # An un-influenced snip contributes nothing but still has to leave
            # the empty per-group slots the evaluation loop iterates over.
            influenced = ddpid is None or ddpid in state._pinfl[0][sp][sn]
            if not influenced:
                pararr[sp].append([[] for _ in range(snip.nea)])
                keyarr[sp].append([[] for _ in range(snip.nea)])
                modtyp[sp].append([[''] for _ in range(snip.nea)])
                continue
            pararr[sp].append([[[] for _ in ea] for ea in snip.slots])
            keyarr[sp].append([[[] for _ in ea] for ea in snip.slots])
            modtyp[sp].append([list(ea) for ea in snip.slots])
            for (i, mtyp, ea, mid) in snip.entries:
                state._funcarray[2][mtyp]._keywd = state._modpass['mkey'][i]
                #print "1.", sp, mtyp, wvrng
                params = state._funcarray[1][mtyp].set_vars(state._funcarray[2][mtyp], p, state._levadd[i], state._modpass, i, wvrng=wvrng, spid=state._specid[sp], levid=state._levadd)
                if len(params) == 0: continue
                if np.size(np.shape(params)) == 1:
                    params = np.array([params])
                    nrow = 1
                else:
                    nrow = np.shape(params)[0]
                slot = pararr[sp][sn][ea][mid]
                if len(slot) != 0 and np.shape(slot[0])[1] != np.shape(params)[1]:
                ...
                slot.append(params)
                keyarr[sp][sn][ea][mid].extend([state._modpass['mkey'][i]] * nrow)
            # One concatenate per slot instead of an np.append per component:
            # the old form recopied the whole block for every row added, which
            # is quadratic in a group's row count (up to 134 on DH_orders).
            # Concatenating the same pieces in the same order is bitwise
            # identical -- it is the same copy, done once.
            for ea in pararr[sp][sn]:
                for mid in range(len(ea)):
                    if len(ea[mid]) == 1:
                        ea[mid] = ea[mid][0]
                    elif len(ea[mid]) > 1:
                        ea[mid] = np.concatenate(ea[mid], axis=0)
################
#
#		pool = mpPool(processes=state.ncpus)
#		pf=[]
#		mydict = state.__dict__.copy()
...
#			pf=[]
#			pool = mpPool(processes=state.ngpus)
...
#                            modelab[sp][ll:lu] *= mout
    ...
    nfitpix = fit_windows(state, x, pos)
    ...
    for sp in range(len(pos)):
        for sn in range(len(pos[sp])-1):
            ...
            if ddpid is not None:
                if ddpid not in state._pinfl[0][sp][sn]:
                    # Stage 4.8: the skipped snip still has to be stepped over
                    # in the packed model vector, but its width is fixed for
                    # the fit -- see build_fit_windows.
                    enf[sp] = stf[sp] + nfitpix[sp][sn]
                    stf[sp] = enf[sp]
                    cvind += 1
                    shind += 1
                    continue
            ...
            params = state._funcarray[1][mtyp].set_vars(state._funcarray[2][mtyp], p, state._levadd[cvind], state._modpass, cvind)
            # Obtain the shift parameters
            shmtyp = state._modpass['mtyp'][shind]
            state._funcarray[2][shmtyp]._keywd = state._modpass['mkey'][shind]
            shparams = state._funcarray[1][shmtyp].set_vars(state._funcarray[2][shmtyp], p, state._levadd[shind], state._modpass, shind)
            shwave = state._funcarray[1][shmtyp].call_CPU(state._funcarray[2][shmtyp], wavespx[sp][llx:lux], shparams, mkey=state._modpass['mkey'][shind])
            mdtmp = state._funcarray[1][mtyp].call_CPU(state._funcarray[2][mtyp], shwave, modelem[sp][llx:lux]*modelab[sp][llx:lux], params, mkey=state._modpass['mkey'][cvind])
...
#					mdtmp = mdtmp +  mzero[sp][llx:lux]*(1.0-mdtmp)/mcont[sp][llx:lux]
            ...
            if state._argflag['out']['wavecorr']:
                outname = state._snipnames[sp][sn].replace(".dat", "_wzcorr.dat")
                wavout = shwave.reshape(x[sp][ll:lu].size,nexbins[sp][sn]).sum(axis=1)/float(nexbins[sp][sn])
            ...
            if state._argflag['run']['capvalue'] is not None:
                wc = np.where(modcv[sp][ll:lu] >= state._argflag['run']['capvalue'])[0]
                if np.size(wc) != 0:
                    modcv[sp][ll:lu][wc] = state._argflag['run']['capvalue']
...
#				modcv[sp][ll:lu] *= state._contfull[sp][ll:lu]
            ...
            state._modfinal[sp][ll:lu][w] = modcv[sp][ll:lu][w]
            state._contfinal[sp][ll:lu][w] = (mcont[sp][llx:lux].reshape(x[sp][ll:lu].size,nexbins[sp][sn]).sum(axis=1)/float(nexbins[sp][sn]))[w]
            state._zerofinal[sp][ll:lu] = (mzero[sp][llx:lux].reshape(x[sp][ll:lu].size,nexbins[sp][sn]).sum(axis=1)/float(nexbins[sp][sn]))
            stf[sp] = enf[sp]
            cvind += 1
            shind += 1
...
def myfunct(p, fjac=None, x=None, y=None, err=None, state=None, output=0, ddpid=None, pp=None, getemab=False, emab=None):
    ...
    if ddpid is None:
        if getemab:
            modconv_fit, emabv = model_func(state, state._wavefull, p, state._posnfull, ddpid=ddpid, output=output, getemab=getemab)
        else:
    ...
    if output == 1:
        state._modconv_all = modconv_fit
        return modf
    elif output == 2:
        return modf
    elif output == 3:
